...
<body class="hold-transition sidebar-mini layout-fixed">
	<div class="wrapper">
		...
		<aside class="main-sidebar sidebar-dark-primary elevation-4">
			<a href="${pageContext.request.contextPath}/UserManual" class="brand-link"> <img
				...
				<span class="brand-text font-weight-light">Thư Viện</span>
			</a>
    ...
    %>
        <div class="info">
            <!-- Hiển thị fullName và điều hướng tới home.jsp khi người dùng đã đăng nhập -->
            <a href="${pageContext.request.contextPath}/home.jsp" class="d-block">Welcome, <%= user.getFullName() %></a>
        </div>
    <%
        } else {
    %>
...
<ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
    ...
    <div style="margin-top: auto;">
        <li class="nav-item">
            <a href="#" class="nav-link" id="logout-link" onclick="toggleLogoutText(event)">
                <!-- Hình ảnh -->
                <img src="Resources/img/logout.png" alt="Logout" class="nav-icon" style="width: 20px; height: 20px; margin-right: 8px;">
                
                <!-- Chữ đăng xuất ẩn -->
                <span id="logout-text" style="display:none;" onclick="logout(event)">Đăng xuất</span>
            </a>
        </li>
    </div>
</ul>
...
<script>
    function toggleLogoutText(event) {
        event.preventDefault();

        var logoutText = document.getElementById('logout-text');

        if (logoutText.style.display === "inline") {
            logoutText.style.display = "none";  // Ẩn chữ "Đăng xuất" khi nhấn vào ảnh
        } else {
            logoutText.style.display = "inline";  // Hiển thị chữ "Đăng xuất"
        }
    }
